...
class AuthCache
{
    ...
    public function setAuthCache($phone, $uuid, $token, $data)
    {
    ...
    // 获取登录验证后的用户信息
    public function getAuthCache($uuid)
    {
    ...
    // 用于注销上一台设备登录验证的信息
    public function delAuthCacheByPhone($phone)
    {
    ...
    // 用于注销本次登录验证信息
    public function delAuthCacheByUuid($uuid)
    {
        $token = $this->_redis->hget($this->_cacheCookieKey, $uuid);
